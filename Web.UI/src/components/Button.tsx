...
export const ButtonDropdown = ({ assos, action }: { assos: any, action: Function }) => {
    const [options, setOptions] = React.useState<Array<{ id: number, name: string }>>([]);
    ...
    const handleClick = () => {
        ...
        // post in particular association
    };
    ...
    return (
        <React.Fragment>
            <ButtonGroup variant="contained" ref={anchorRef} aria-label="split button">
                <Button onClick={handleClick}>{options.length > 0 ? options[selectedIndex].name : 'Loading..'}</Button>
                <Button
                    size="small"
